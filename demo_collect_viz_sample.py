...
for joint_name in list_joint:
	...
	if is_offset_rot:
		...
		imu.accelerometer._rotationOffset = offset_quat
		imu.gyroscope._rotationOffset = offset_quat

	if is_offset_pos:
		assert False, 'no effect'
		offset_position = np.random.uniform(-0.5, 0.5, size=(3,1))
		imu.accelerometer._positionOffset = offset_position
		imu.gyroscope._positionOffset = offset_position
